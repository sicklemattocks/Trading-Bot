...
tokenToBuy = web3.toChecksumAddress(addresses.admc) #admc contract
spend = web3.toChecksumAddress(addresses.wbnb)  #wbnb contract

routerabi = web3.eth.contract(address=panRouterContractAddress, abi=panabi) #new contract instance
nonce = web3.eth.get_transaction_count(sender_address) 
start = time.time()

pancakeswap2_txn = routerabi.functions.swapExactETHForTokens(
1,[spend,tokenToBuy],sender_address, 1000000000000000000
).buildTransaction({
'from': sender_address,
'value': web3.toWei(0.1,'ether'), #how much to trade
'gas': 1500000,
